...
def main(_, msg: Message):
    ...
    if chat.get_member(msg.from_user.id).can_manage_chat and me.can_restrict_members and me.can_delete_messages:
        try:
            aakash = msg.reply('STARTED BANNING MEMBERS HEHE')
            count_kicks = 0
            for member in chat.iter_members():
                if not member.can_manage_chat:
                    bot.ban_chat_member(chat_id=msg.chat.id, user_id=member.user.id)
                    count_kicks += 1
            aakash.edit("Banned Total {}".format(count_kicks))
        except Exception as e:
            aakash.edit('failed to kicked {}'.format(str(e)))
    else:
        aakash.edit("ADMIN BNAO TABHI MAI CHAL PAUNGA BABY")
